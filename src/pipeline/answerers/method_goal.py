...
class QuestionAnswererMethodGoal:
    ...
    def answer_method_goal_question(self, question: QuestionAnswerRecipe) -> Tuple[str, Dict[str, str]]:

        """
        Answers the question "how do you", belonging to method Goal (original 14). Iterate over paragraphs and semantic
        role's columns to find specified labels and relations.
        :param question: question to be answered
        :return: answer
        """
        more_info_for_answer = {"source": QuestionAnswererMethodGoal.DESCRIPTION}
        ...
        if answer:
            more_info_for_answer["details_for_excel"] = "relation || class MethodGoal"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        answer = self.steps_and_columns_iteration(question.recipe, question_changed, paragraphs)

        if answer:
            more_info_for_answer["details_for_excel"] = "semantics || class MethodGoal"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        more_info_for_answer["details_for_excel"] = "no match || class MethodGoal"
