...
class QuestionAnswerer8(QuestionAnsweringBase):
    DESCRIPTION = "QuestionAnswerer: Where do you?"
    ...
    def make_use_of_drop(self, drop_column: str) -> str:
        ...
        drop_column = [self.inflection.singular_noun(value) if self.inflection.singular_noun(value) else value for value in drop_column]
        drop_column = [value.replace("-", " - ") for value in drop_column]
    ...
    def steps_and_columns_iteration(self, recipe: Recipe, question: str, paragraphs: List[str],
        ...
        for paragraph in paragraphs:
            for annotation_column in range(0, 10):
                ...
                if use_drop_column:
                    answer = self.verb_iteration(annotation_column, recipe, question, iter_type, verbs, use_drop_column=True)
                    if answer:
                        return answer
                    ...
                    if semantics:
                        ...
                        answer = self.verb_iteration(annotation_column, recipe, question, iter_type, verbs, semantics=False)
                        if answer:
                            return answer
    ...
    def answer_a_question(self, question: QuestionAnswerRecipe, question_category: QuestionCategory,
        ...
        """
        Answers the question "where do you", belonging to class 8, 12 and 13. Iterate over paragraphs and semantic
        role's columns to find specified labels.
        :param question: question to be answered
        :param question_category: assumed to be 8
        :param more_info: ignored
        :return: answer
        """
        more_info_for_answer = {"source": QuestionAnswerer8.DESCRIPTION}
