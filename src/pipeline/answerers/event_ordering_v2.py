...
from src.pipeline.answerers.event_ordering_v1 import _return_answer
...
class QuestionAnswererEventOrdering(QuestionAnsweringBase):
    ...
    def answer_a_question(self, question: QuestionAnswerRecipe, question_category: QuestionCategory,
        ...
        :return:
        """
        more_info_for_answer = {"source": QuestionAnswererEventOrdering.DESCRIPTION}
        ...
        first_event: EoEvent = self.extract_events_from_segment(segments[0])
        last_event: EoEvent = self.extract_events_from_segment(segments[-1])
        ...
        if not final_answer:
            ...
            last_match_1 = last_match_1 if last_match_1 \
                else QuestionAnswererEventOrdering.soft_match(all_events, first_event)
            print(f"{rule_applied}1 = {last_match_1}", file=outstream)
            last_match_2 = last_match_2 if last_match_2 \
                else QuestionAnswererEventOrdering.soft_match(all_events, last_event)
            print(f"{rule_applied}2 = {last_match_2}", file=outstream)
            final_answer = self.compare_by_positions(last_match_1, last_match_2, outstream)
        ...
        if not final_answer:
            ...
            last_match_1 = last_match_1 if last_match_1 \
                else QuestionAnswererEventOrdering.verb_match(all_events, first_event)
            last_match_2 = last_match_2 if last_match_2 \
                else QuestionAnswererEventOrdering.verb_match(all_events, last_event)
        ...
        if not final_answer:
            ...
            final_answer = self.handle_stupid_cases(first_event, last_event)
    ...
    @staticmethod
    def verb_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
        return [event for event in all_events if reference_event.is_verb_match(event)]

    @staticmethod
    def exact_object_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
        return [event for event in all_events if reference_event.is_exact_object_match(event)]

    @staticmethod
    def soft_object_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
        return [event for event in all_events if reference_event.is_soft_object_match(event)]

    @staticmethod
    def soft_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
        return [event for event in all_events if reference_event.is_soft_match(event)]

    @staticmethod
    def exact_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
        return [event for event in all_events if reference_event.is_exact_match(event)]

    @staticmethod
    def full_sentence_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) -> List[VerbPatientHabitat]:
    ...
    @staticmethod
    def object_to_verb_match(all_events: List[VerbPatientHabitat], reference_event: EoEvent) \
    ...
    @staticmethod
    def handle_stupid_cases(event1: EoEvent, event2: EoEvent) -> str:
        """
    ...
    def extract_events_from_segment(self, segment: List[str]) -> EoEvent:
        ...
        return EoEvent(lemmatized_verb, rets, segment)
