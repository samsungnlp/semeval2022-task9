...
class QuestionAnswererMethodInstrument:
    ...
    def answer_class_method_instrument_question(self, question: QuestionAnswerRecipe) -> Tuple[str, Dict[str, str]]:

        """
        Answers the question "how do you", belonging to class 6 / method instrument.
        Iterate over paragraphs and semantic role's columns to find specified labels and relations.
        :param question: question to be answered
        :return: answer
        """
        more_info_for_answer = {"source": QuestionAnswererMethodInstrument.DESCRIPTION}
        ...
        if answer:
            more_info_for_answer["details_for_excel"] = "relation || Method / Instrument"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        answer = self.steps_and_columns_iteration(question.recipe, question_changed, paragraphs)

        if answer:
            more_info_for_answer["details_for_excel"] = "semantics || Method / Instrument"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        more_info_for_answer["details_for_excel"] = "no match || Method / Instrument"
