...
class QuestionAnswerer2Where(QuestionAnsweringBase):
    ...
    def answer_a_question(self, question: QuestionAnswerRecipe, question_category: QuestionCategory,
        ...
        :return:
        """
        more_info_for_answer = {"source": QuestionAnswerer2Where.DESCRIPTION}
        self.outstream = io.StringIO()
    ...
    def get_final_from_candidates(self, candidates: List[str]) -> str:
        ...
        if len(ret) >= 2:
            print(f"Non-unique answer to C2 = {ret}", file=self.outstream)
            c = Counter()
            for item in ret:
                c[item] += 1
