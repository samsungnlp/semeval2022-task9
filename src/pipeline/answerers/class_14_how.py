...
import inflect
from src.utils_class_2_6_10_14 import WordMistakesRepair


class QuestionAnswerer14How:
    ...
    def concat_words(sentence: AnnotatedSentence, annotation_column: int,
        ...
        for token_idx, token in enumerate(sentence.annotated_tokens):
            ...
            if semantic_role == f"B-{possible_answer}":
                b_answer_found = True
                extra_idx = token.id
                words[(sentence.sentence_id, sentence.paragraph_id, extra_idx)] = [value]
            ...
            # If f"B-{possible_answer}" has been found, we append f"I-{possible_answer}"s if we find them
                ...
                words[(sentence.sentence_id, sentence.paragraph_id, extra_idx)].append(value)
    ...
    def answer_class14_question(self, question: QuestionAnswerRecipe) -> Tuple[str, Dict[str, str]]:

        """
        Answers the question "how do you", belonging to class 14. Iterate over paragraphs and semantic
        role's columns to find specified labels and relations.
        :param question: question to be answered
        :return: answer
        """
        more_info_for_answer = {"source": QuestionAnswerer14How.DESCRIPTION}
        ...
        if answer:
            more_info_for_answer["details_for_excel"] = "relation || class 14"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        answer = self.steps_and_columns_iteration(question.recipe, question_changed, paragraphs)

        if answer:
            more_info_for_answer["details_for_excel"] = "semantics || class 14"
            return self.mistakes.change_answer(question.question.lower(), answer), more_info_for_answer

        more_info_for_answer["details_for_excel"] = "no match || class 14"
