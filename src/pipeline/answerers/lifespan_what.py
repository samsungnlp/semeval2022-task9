...
class QuestionAnswererLifespanWhat(QuestionAnsweringBase):
    DESCRIPTION = "QuestionAnswerer: What_is_in?"
    ...
    def answer_a_question(self, question: QuestionAnswerRecipe, question_category: QuestionCategory,
        ...
        """
        Answers the question "What's in the ..." / lifespan (what)

        :param question: question to be answered
        :param question_category: assumed to be Lifespan (what)
        :param more_info: ignored
        :return: answer about contents
        """
        more_info_for_answer = {"source": QuestionAnswererLifespanWhat.DESCRIPTION}
    ...
    def retrieve_answer(recipe: AnnotatedRecipe, sentence_idx: Any, token_idx: Any) -> Any:
        ...
        allowed_roles = {"I-Patient", "B-Patient", "B-Co-Patient", "I-Co-Patient", "I-Theme", "B-Theme", "I-Co-Theme",
                         None}

        sentence = recipe.annotated_sentences[sentence_idx]

        relevant_tokens = sentence.annotated_tokens[token_idx:]
        verb_position = QuestionAnswererLifespanWhat.which_in_turn_verb_in_sentence(relevant_tokens[0])
        ...
        for token in relevant_tokens[1:]:
            ...
            role = None if verb_position is None else token.semantic_roles[
                verb_position - 1]  # because they are 0-based
        ...
        if drop_information:
            ...
            contents = QuestionAnswererLifespanWhat.remove_duplicates_from_drop(contents, drop_phrases)
            contents += drop_phrases

        if not contents:
            return None

        answer = QuestionAnswererLifespanWhat.concatenate_words_into_phrase(contents)
